...
update  geotools.bldgs b set cl_id = 'unk'/*, sts=CASE WHEN sts='clustered' THEN 'cluster' ELSE sts END*/
   where cl_id <>'unk' and loc=in_loc  and rev=in_rev;
$$;
...
FROM (
       ...
       WHERE
         b.sts IN ('cluster', 'clustered') AND b.loc = in_loc AND b.rev = in_rev AND cl.loc = in_loc AND cl.rev = in_rev
     ) x
...
FROM (
       ...
       WHERE
         ...
         AND b.loc = in_loc          AND b.rev = in_rev
...
CREATE OR REPLACE FUNCTION geotools.insert_pole_nodes( in_loc TEXT, in_rev TEXT )
  RETURNS BIGINT AS
$$
DECLARE
  rt_val BIGINT :=0;
 ...
 DELETE  FROM geotools.nodes where  loc = in_loc AND rev = in_rev;
  ...
  WITH q10 AS (  --3
      SELECT
        ...
        min(snap_geom)  geom
      FROM geotools.bldgs b
      WHERE b.cl_id <> 'unk' AND hhr >= 0.1 AND b.loc = in_loc AND b.rev = in_rev
      GROUP BY snap_geom
  )

    , q_insert as(
  INSERT INTO geotools.nodes ( loc, rev, hh_cnt, b_cnt, cl_id, node_type, geom)
    (SELECT       in_loc,       in_rev,       hh_cnt,       b_cnt,       cl_id,       'pole',       geom     FROM q10)
...
$$ LANGUAGE 'plpgsql';
--####################
CREATE OR REPLACE FUNCTION geotools.update_tr_nodes( in_loc TEXT, in_rev TEXT )
  RETURNS BIGINT AS
$$
DECLARE
  rt_val BIGINT :=0;
BEGIN
 --DELETE  FROM geotools.nodes where  loc = in_loc AND rev = in_rev;
  update geotools.nodes n set  node_type = 'pole'  where  loc = in_loc AND rev = in_rev and n.node_type ='tr_pole';
  WITH q10 AS (  --3
      SELECT
      n.objectid
      FROM geotools.bldgs_cl cl join
        geotools.nodes n on st_dwithin(
          cl.ce_geom, n.geom,1
        )
      WHERE  cl.loc = in_loc AND cl.rev = in_rev and  n.loc = in_loc AND n.rev = in_rev

  )

    , q_update as(
  update geotools.nodes n
  set node_type ='tr_pole' from q10 q where n.objectid = q.objectid
     RETURNING 1)
...
--#############################
--#############################
CREATE OR REPLACE FUNCTION geotools.insert_pole_to_tr_edges(in_loc TEXT, in_rev TEXT, cost_pow NUMERIC DEFAULT 0)
  RETURNS BIGINT AS
$$
DECLARE
  rt_val BIGINT :=0;
BEGIN
  ...
  WITH q10 AS (
      SELECT
        ...
        tr.loc                           loc,
        tr.rev,
        degrees(ST_Azimuth(tr.geom,p.geom)) bearing_deg,
        row_number() OVER (partition by tr.cl_id, ceil( (1+degrees(ST_Azimuth(tr.geom,p.geom)))/90)
          ORDER BY st_distance(tr.geom,p.geom)) rnk,
      ...
      and tr.objectid not in (    select n1.objectid from geotools.nodes n1 join geotools.roads rd1 on st_dwithin(n1.geom, rd1.geom, 1)
      where n1.node_type='tr_pole' and n1.loc = in_loc and n1.rev = in_rev and rd1.loc &&array[ in_loc ]
...
--      and p1.objectid not in (    select n1.objectid from geotools.nodes n1 join geotools.roads rd1 on st_dwithin(n1.geom, rd1.geom, 1)
--       where n1.node_type='pole' and n1.loc = in_loc and n1.rev = in_rev and rd1.loc &&array[ in_loc ])
  )
      , q30 as (
       SELECT
        cl_id,
        ...
        row_number() OVER (PARTITION BY source, ceil( (1+degrees(ST_Azimuth( st_startpoint(geom), st_endpoint(geom)))/90))  ORDER BY st_length (geom) ASC ) rnk,
        geom
      FROM q10 l
    )
    , q_insert AS (
...
BEGIN
  ...
  WITH q10 AS (
    ...
    (cl_id, source, target, dist, "cost", cost_mult, src_trgt_type, loc, rev, geom)
      SELECT cl_id, source, target, dist, "cost", cost_mult, src_trgt_type, loc, rev, geom
      FROM q10
      WHERE rnk = 1 and  dist > 0 ON CONFLICT DO NOTHING
    RETURNING 1 cnt
  )
  SELECT count(cnt)
  INTO rt_val
...
BEGIN
  ...
  WHERE met.loc = in_loc AND met.rev = in_rev AND met.src_trgt_type = 'rd_to_rd';
    ...
    ( cl_id, source, target, dist, "cost", cost_mult, src_trgt_type, loc, rev, geom)
      SELECT *
      FROM q40 on conflict do nothing;
  /*
    , q_edge_insert AS (
    INSERT INTO geotools.master_edge_table
    ( cl_id, source, target, dist, "cost", cost_mult, src_trgt_type, loc, rev, geom)
...
BEGIN
  ...
  WHERE met.loc = in_loc AND met.rev = in_rev AND met.src_trgt_type = 'rd_line_to_common';
     ...
     with  q40 AS (
      ...
      FROM   geotools.nodes n
         WHERE n.node_type in('line', 'rd_line')
...
BEGIN
  ...
  UPDATE geotools.master_edge_table set cost_mult= 1 where cost_mult < 1 ;
        ...
        WITH q10 AS (
          ...
          where cost_mult>0
            ...
            (objectid, source, target, cl_id, dist, "cost", v_level, src_trgt_type, loc, rev, geom)
              ...
              FROM q20
            RETURNING *)
                   UPDATE geotools.master_edge_table met set cost_mult=cost_mult*0.5
              from q_insert where met.objectid=q_insert.objectid ;
...
CREATE OR REPLACE FUNCTION geotools.set_tr_size(in_tr_sizes numeric[], in_tr_size_unit text, in_loc TEXT, in_rev TEXT)
  ...
  BEGIN
    ...
    , q20 as (
        ...
        from geotools.bldgs_cl cl
          JOIN q10 tr on tr.ts*0.5 > cl.demand
...
-----
CREATE OR REPLACE FUNCTION geotools.snap_bldg_node_to_roads(in_loc TEXT, in_rev TEXT, in_dist INTEGER)
  RETURNS integer
AS $$
DECLARE
  rt_value INTEGER :=0;
BEGIN
  WITH q10 AS (
      SELECT
        ...
        row_number()
        OVER (
          PARTITION BY n.objectid
          ORDER BY n.geom <-> r.geom ASC ) rnk
      FROM geotools.bldgs n
        JOIN geotools.roads r ON st_dwithin(n.geom, r.geom, in_dist)
      WHERE n.loc = in_loc AND n.rev = in_rev
            AND r.loc && ARRAY [in_loc]
  )

    , q20 AS (
      SELECT
        n.objectid,
        CASE WHEN (n.n_geom <-> st_startpoint(r_geom)) > (n.n_geom <-> st_endpoint(r_geom))
          THEN
            st_endpoint(r_geom)
        ELSE st_startpoint(r_geom) END geom_snapped

      FROM q10 n
  )
    , q_update AS (

    UPDATE geotools.bldgs n
    SET snap_geom = q.geom_snapped
    FROM q20 q
    WHERE n.objectid = q.objectid
--    ON CONFLICT DO NOTHING
    RETURNING 1
...
--------------
-----
CREATE OR REPLACE FUNCTION geotools.snap_cegeom_to_roads(in_loc TEXT, in_rev TEXT, in_dist INTEGER)
  RETURNS integer
AS $$
DECLARE
  rt_value INTEGER :=0;
BEGIN
  WITH q10 AS (
      SELECT
        ...
        OVER (
          PARTITION BY n.objectid
          ORDER BY n.ce_geom <-> r.geom ASC ) rnk
      FROM geotools.bldgs_cl n
        JOIN geotools.roads r ON st_dwithin(n.ce_geom, r.geom, in_dist)
      WHERE n.loc = in_loc AND n.rev = in_rev
            AND r.loc && ARRAY [in_loc]
  )
    ...
    UPDATE geotools.bldgs_cl n
    SET ce_geom = q.geom_snapped
    FROM q20 q
    WHERE n.objectid = q.objectid
--    ON CONFLICT DO NOTHING
    RETURNING 1
...
--- start mv routing------------
--#########################################################################
CREATE OR REPLACE FUNCTION geotools.insert_tr_to_rd_out_cl_edges(in_buf_rad numeric, in_loc TEXT, in_rev TEXT,cost_pow NUMERIC DEFAULT 4)
  RETURNS BIGINT AS
$$
DECLARE
  rt_val BIGINT :=0;
BEGIN
  ...
  WITH q10 AS  (
      SELECT
        ...
        p.objectid                       target,
        st_distance(tr.geom, p.geom)     dist,
       greatest((st_distance(tr.geom, p.geom)-in_buf_rad),0) ^ cost_pow + st_distance(tr.geom, p.geom) "cost",
        1                                cost_mult,
        'tr_to_rd'                     src_trgt_type,
        tr.loc                           loc,
        tr.rev,
        row_number() OVER (PARTITION BY tr.objectid ORDER BY st_distance(tr.geom,p.geom)) rnk,
        st_makeline(tr.geom, p.geom)     geom
      FROM geotools.nodes p
        JOIN geotools.nodes tr ON st_dwithin(tr.geom,  p.geom, 10000)
      WHERE p.node_type in( 'rd' , 'rd_line') AND tr.node_type = 'tr_pole'
            AND p.loc = in_loc AND p.rev = in_rev
            AND tr.loc = in_loc AND tr.rev = in_rev
          and tr.objectid not in (    select n1.objectid from geotools.nodes n1 join geotools.roads rd1 on st_dwithin(n1.geom, rd1.geom, 1)
...
--******************************************************************************
CREATE OR REPLACE FUNCTION geotools.insert_tr_to_tr_edges(in_buf_rad numeric, in_loc TEXT, in_rev TEXT, cost_pow NUMERIC DEFAULT 4)
  RETURNS BIGINT AS
$$
DECLARE
  rt_val BIGINT :=0;
BEGIN
  DELETE FROM geotools.master_edge_table met
  WHERE met.loc = in_loc AND met.rev = in_rev AND met.src_trgt_type = 'tr_to_tr';
  ...
  WITH q10 AS (
      SELECT
        ...
        case when p1.objectid < p2.objectid then st_makeline(p1.geom, p2.geom) else st_makeline(p2.geom, p1.geom) END   geom
      FROM geotools.nodes p1
        JOIN geotools.nodes p2 ON p2.node_type = 'tr_pole'  and p1.objectid<>p2.objectid

      WHERE p1.node_type = 'tr_pole'
            AND p1.loc = in_loc AND p1.rev = in_rev
            AND p2.loc = in_loc AND p2.rev = in_rev
           -- AND p1.cl_id = ''
  )
      , q30 as (
       SELECT
        ...
        target,
        st_length(geom) dist,
        greatest((st_length(geom)-2*in_buf_rad),0) ^ cost_pow + 2*st_length(geom) "cost",
        1                                cost_mult,
        'tr_to_tr'                     src_trgt_type,
        row_number() OVER (PARTITION BY source/*,ceil( (1+degrees(ST_Azimuth( st_startpoint(geom), st_endpoint(geom)))/90)) */ ORDER BY st_length (geom) ASC ) rnk,
        geom
      FROM q10 l
    )
...
--#############################################################################
CREATE OR REPLACE FUNCTION geotools.insert_nodes_tr_to_lines(in_loc TEXT, in_rev TEXT)
  RETURNS BIGINT AS
$$
DECLARE
  rt_val BIGINT :=0;
BEGIN
DELETE FROM geotools.nodes where rev=in_rev and loc=in_loc and node_type='line';

  WITH q10 AS (
      SELECT
        tr.cl_id,
        row_number() OVER (PARTITION BY tr.objectid ORDER BY tr.geom<->l.geom ASC) rnk,
        st_makeline(tr.geom, l.geom)     geom
      FROM geotools.nodes tr
        JOIN geotools.lines l ON st_dwithin(tr.geom,  l.geom, 10000)
      WHERE tr.node_type = 'tr_pole'
            AND tr.loc = in_loc AND tr.rev = in_rev
  ), q_insert AS(
    INSERT INTO geotools.nodes ( loc, rev, hh_cnt, b_cnt, cl_id, node_type, geom)
      SELECT  in_loc, in_rev, 0, 0, cl_id, 'line', st_endpoint(geom)
          FROM q10
      WHERE rnk=1 on CONFLICT do  NOTHING
        RETURNING 1
    )
...
----***
--#########################################################################
CREATE OR REPLACE FUNCTION geotools.insert_tr_to_lines_edges(in_buf_rad numeric,in_loc TEXT, in_rev TEXT,cost_pow NUMERIC DEFAULT 4)
  RETURNS BIGINT AS
$$
DECLARE
  rt_val BIGINT :=0;
BEGIN
  delete from geotools.master_edge_table where loc=in_loc and rev = in_rev and src_trgt_type='tr_to_line';
  WITH q10 AS (
      SELECT
        tr.cl_id,
        tr.objectid                      source,
        ...
        greatest((st_distance(tr.geom, p.geom)-in_buf_rad),0) ^ cost_pow + st_distance(tr.geom, p.geom) "cost",
        1                                cost_mult,
        'tr_to_line'                     src_trgt_type,
        tr.loc                           loc,
        tr.rev,
        st_makeline(tr.geom, p.geom)     geom
      FROM geotools.nodes p
        JOIN geotools.nodes tr ON p.cl_id=tr.cl_id
      WHERE p.node_type in( 'line') AND tr.node_type = 'tr_pole'
            AND p.loc = in_loc AND p.rev = in_rev
            AND tr.loc = in_loc AND tr.rev = in_rev
           -- AND p.cl_id = ''
  )
...
--#############################################################################
CREATE OR REPLACE FUNCTION geotools.insert_rd_to_line_edges(in_loc TEXT, in_rev TEXT,cost_pow NUMERIC DEFAULT 4)
  RETURNS BIGINT AS
$$
DECLARE
  rt_val BIGINT :=0;
BEGIN
    delete from geotools.master_edge_table where loc=in_loc and rev = in_rev and src_trgt_type='rd_to_line';
  ...
  WITH q10 AS (
      SELECT
        ...
        l.objectid                      target,
        p.objectid                       source,
        st_distance(l.geom, p.geom)     dist,
        st_distance(l.geom, p.geom) ^ cost_pow "cost",
        1                                cost_mult,
        'rd_to_line'                     src_trgt_type,
        l.loc                           loc,
        l.rev,
        row_number() OVER (PARTITION BY l.objectid ORDER BY st_distance(l.geom,p.geom)) rnk,
        st_makeline(p.geom, l.geom)     geom
      FROM geotools.nodes p
        JOIN geotools.nodes l ON st_dwithin(l.geom,  p.geom, 10000)
      WHERE p.node_type= 'rd'  AND l.node_type = 'line'
            AND p.loc = in_loc AND p.rev = in_rev
            AND l.loc = in_loc AND l.rev = in_rev
           -- AND p.cl_id = ''
  )
    , q_insert AS (
    INSERT INTO geotools.master_edge_table
...
LOOP
   RAISE NOTICE 'Current object id : %', l_objectid;
...
with RECURSIVE q10 (prj,objectid,ar_objectid,source,target) as (
  SELECT
    ...
    target
  from geotools.new_lines l
  WHERE l.objectid = l_objectid
  UNION
    SELECT   prj,
    nl.objectid objectid, ar_objectid||array[nl.objectid] ar_object, nl.source,nl.target
    from geotools.new_lines  nl
    JOIN q10 r on ( r.source = nl.source or r.source = nl.target or ( r.target = nl.source or r.target = nl.target) ) and not ar_objectid && ARRAY[nl.objectid] and nl.target<>0 and nl.source<>0 and nl.v_level='mv'
   where     nl.loc=in_loc and nl.rev=in_rev
)
  ,q_update as(
  UPDATE geotools.new_lines l SET proj=q.prj
FROM (
select * from q10 )q
WHERE q.objectid=l.objectid
  returning 1
    )
    SELECT  count(*)+rt_val into rt_val from q_update;
  END LOOP ;
